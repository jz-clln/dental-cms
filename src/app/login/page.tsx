...
export default function LoginPage() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    const { error: otpError } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      options: {
        // This tells Supabase NOT to create a new user if the email doesn't exist
        // Existing users get a login link, new users should use /signup
    ...
    if (otpError) {
      // "Email not confirmed" or "User not found" — tell them to sign up
      if (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleMagicLink} className="space-y-4">
            ...
            >
              {loading
                ...
                : <><Mail className="w-4 h-4" /> Send me a login link</>
              }
            </button>
          </form>
        ...
        <div className="mt-4 bg-white/60 border border-gray-100 rounded-xl px-5 py-3.5 flex items-start gap-3">
          <CheckCircle className="w-4 h-4 text-teal-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-xs font-semibold text-gray-700">No password required</p>
            <p className="text-xs text-gray-400 mt-0.5">
              We send a secure one-time link to your email. Safer than a password — nothing to forget or lose.
            </p>
          </div>
        </div>
